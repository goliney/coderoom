...
function copyMedia() {
    ...
    options.css = css_files.map(file => 'media/' + path.resolve(file).slice(commonMediaPath.length).replace(/\\/g, '/'));
    options.js = js_files.map(file => 'media/' + path.resolve(file).slice(commonMediaPath.length).replace(/\\/g, '/'));
}
